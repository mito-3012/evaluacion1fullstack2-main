...
// ==========================================
function rutaImagenProducto(imagen) {
    if (!imagen) return "../play5.jpg";
    // Las imágenes están en la raíz del proyecto y las páginas HTML están dentro de /html.
    // Si ya viene con una ruta relativa/absoluta, la respetamos.
    if (imagen.startsWith("../") || imagen.startsWith("/") || imagen.startsWith("http://") || imagen.startsWith("https://")) {
        return imagen;
    }
    return "../" + imagen;
}
...
function renderizarCatalogoGamer() {
    const gridContainer = document.getElementById("productos-container") || document.getElementById("products-grid") || document.getElementById("all-products-container");
    ...
    gridContainer.innerHTML = productos.map(p => `
        <div class="product-card">
            <a href="detalle-producto.html?id=${p.id}">
                <div class="img-box">
                    <img src="${rutaImagenProducto(p.imagen)}" alt="${p.nombre}" onerror="this.src='../play5.jpg'">
                </div>
            </a>
            ...
            <div style="display: flex; gap: 8px; margin-top: 12px;">
                <a href="detalle-producto.html?id=${p.id}" class="btn btn-outline" style="flex: 1;">Ver Detalle</a>
...
function cargarDetalleProducto() {
    ...
    if (pImg) {
        pImg.src = rutaImagenProducto(prod.imagen);
        pImg.onerror = () => { pImg.src = "../play5.jpg"; };
    }
...
function renderizarCarritoGamer() {
    ...
    cartList.innerHTML = cart.map(item => {
        ...
        // Si un producto fue agregado al carrito antes de guardar su imagen,
        // la recuperamos desde el catálogo usando su ID.
        const productoCatalogo = productos.find(p => p.id === Number(item.id));
        const imagenProducto = item.imagen || (productoCatalogo ? productoCatalogo.imagen : "play5.jpg");

        return `
            <div class="cart-item">
                <div style="display: flex; align-items: center; gap: 15px; min-width: 0;">
                    <img src="${rutaImagenProducto(imagenProducto)}" alt="${item.nombre}" style="width: 80px; height: 80px; object-fit: cover; border-radius: 8px; flex-shrink: 0;" onerror="this.src='../play5.jpg'">
